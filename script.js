...
let dropdown=document.querySelectorAll('.form select')
...
for (let i=0;i<dropdown.length;i++) {
    for ( currcode in countryList) {
        ...
        if(dropdown[i].name=='from' && currcode=='PKR'){
          option.selected='selected'
        }
        else if(dropdown[i].name=='to' && currcode=='USD'){
          option.selected='selected'
        }
...
button.addEventListener('click',async(e)=>{
    ...
    let URL=`https://v6.exchangerate-api.com/v6/c2d5ed0e7c729ba3387f51b1/latest/${fromcurr.value}`
